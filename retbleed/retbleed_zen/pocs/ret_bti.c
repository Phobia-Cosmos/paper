...
// When training
// TODO:什么叫做when training?为什么地址的末尾是UL?0x41bababababfUL又是什么?
...
// TODO:这样声明的函数是不是仅仅为了提供一个地址?函数声明紧跟着汇编代码定义是不是代表函数内部的内容?汇编代码中的标号就是函数名称会有什么效果?
/**
...
int main(int argc, char *argv[])
{
    ...
    // collisions on.
    map_or_die((void *)(BR_SRC1 & ~0xfff), PG_ROUND((BR_SRC1 & 0xfff) + br_src_training_sz), PROT_RWX, MMAP_FLAGS, -1, 0);
    ...
    for (int nbits = 1; nbits <= MAX_MUTATIONS; ++nbits)
    {
        ...
        while (ptrn < (1UL << (MAX_BIT - SKIP_LOWER_BITS)))
        {
            ...
            if (mmap((void *)(br_src_mispredict & ~0xfff),
                     br_src_mispredict_map_sz, PROT_RWX, MMAP_FLAGS, -1, 0) == MAP_FAILED)
            {
            ...
            for (int i = 0; i < ROUNDS; ++i)
            {
                ...
                asm(
                    ...
                    ".rept " xstr(RET_PATH_LENGTH) "\n\t"
                                                   ...
                                                   ".endr\n\t"
                                                   "ret\n\t" ::"r"(train_dst),
                    [retp] "r"(training_path), [br_src] "r"(BR_SRC1) : "rax", "rdi", "r8", "r9", "r10");
                ...
                asm(
                    ...
                    "mov $" xstr(SECRET) ", %%rdi\n\t"
                                         ...
                                         ".rept " xstr(RET_PATH_LENGTH) "\n\t"
                                                                        ...
                                                                        ".endr\n\t"
                                                                        "ret\n\t" ::"r"(spec_dst),
                    [retp] "r"(speculation_path), [br_src] "r"(br_src_mispredict) : "rax", "rdi", "r8", "r9", "r10");
                printf("round\n");
